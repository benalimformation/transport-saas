...
function ModifierDepenseContent({ params }: { params: { id: string } }) {
  ...
  async function chargerDepense(id: string, idEntreprise: string) {
    ...
    try {
      ...
      if (data) {
        ...
        setError("Dépense non trouvée ou vous n'avez pas les droits pour la modifier.");
      }
    } catch (err) {
  ...
  async function soumettreFormulaire(e: React.FormEvent) {
    ...
    try {
      ...
      // Mise à jour de la dépense existante avec double filtre
  ...
  return (
    <main className="min-h-screen bg-gray-950 p-10 text-white">
      <div className="mb-8 flex items-center justify-between">
        <h1 className="text-5xl font-bold">Modifier Dépense</h1>
...
export default function ModifierDepensePage({ params }: { params: { id: string } }) {
  return (
    <Suspense fallback={<div>Chargement...</div>}>
      <ModifierDepenseContent params={params} />
    </Suspense>
  );
}