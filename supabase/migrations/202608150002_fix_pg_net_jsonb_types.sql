-- Migration: Fix pg_net JSONB types for Stripe customer provisioning
-- Date: 2026-08-15
-- Description: Corrects the trigger_stripe_customer_creation function to use proper JSONB types for pg_net
--              parameters instead of casting to text. This ensures compatibility with pg_net's expected
--              parameter types while maintaining all existing functionality.

BEGIN;

-- Create or replace the function with corrected pg_net parameter types
...
BEGIN
  ...
  -- Call Edge Function asynchronously via pg_net with corrected JSONB types
